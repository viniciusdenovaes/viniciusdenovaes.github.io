...
A primeira parte avaliada será o _protótipo_: um relatório em jupyter mostrando o resultado do(s) método(s) funcionando. Depois será avaliado o estudo do(s) método(s). Os métodos podem ter sido treinados anteriormente. 

E, por fim, caso o grupo decida fazer, o programa que recebe uma imagem de uma vogal e responda se é um `i` ou não.

- 2pts: Um baseline com as pontuações de _**acurácia média**_, _**precision**_, _**recall**_ e _**f1**_
- 2pts: Um outro algoritmo comparando com o baseline.
- 2pts: Validações cruzadas.
- 2pts: Teste do mesmo algoritmo com testes de mudança de parâmetros.
- 2pts: Uma justificativa de qual método é melhor.
- 2pts (**Bônus**): Um programa que receba uma imagem qualquer com uma vogal escrita à mão e responda se é um `i` ou não.
