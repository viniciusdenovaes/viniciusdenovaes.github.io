...
\section{Raízes de Equações Usando o Método da Bissecção}

Se temos uma função $f(x)$ 
contínua no intervalo $[a, b]$, 
e se $f(a)\cdot f(b) < 0$, ou seja, 
se ($f(a)>0$ e $f(b)<0$) ou 
($f(a)<0$ e $f(b)>0$).

Então podemos ter certeza que temos uma raíz entre os
pontos $a$ e $b$.

Para encontrar esta raíz podemos usar o método da bissecção,
que consiste em fazer uma busca iterativa neste intervalo,
sempre dividindo este intervalo ao meio a cada iteração.

Dada uma função $f(x)$, os pontos $a$ e $b$, 
tal que $f(a)\cdot f(b)<0$, 
e uma tolerância de erro $\epsilon>0$,
podemos seguir os seguintes passos:

\begin{enumerate}
   \item Encontre o ponto médio $c = \frac{a+b}{2}$
   \item Se $|f(c)|<\epsilon$ tratamos o $c$ como a raíz
   \item Caso nao seja,
   \begin{itemize}
      \item Se $f(a)\cdot f(c)<0$ a raíz está entre $a$ e $c$,
      recomece a iteração com $c$ no lugar de $b$
      \item Se $f(b)\cdot f(c)<0$ a raíz está entre $b$ e $c$
      recomece a iteração com $c$ no lugar de $a$
   \end{itemize}
\end{enumerate}


\textbf{Exercício}: 
Dada a função $f(x) = -x + 1.1$ e os pontos $0$ e $8$, 
com $\epsilon=0.15$, 
execute o procedimento escrito e escreva os valores de $c$.
